...
`load_nifti_and_stem.py` automatically applies the correct Z-rotations: Mathys and Medacta AMISTEM stems receive `--post-rotate-z-180`, while Johnson Corail/Actis stems get both `--pre-rotate-z-180` and `--post-rotate-z-180`. Pass `--rotation-mode mathys|medacta|johnson|none` to override.
...
Each metrics XML now corresponds to a specific stem configuration, so the `Cases` worksheet records the configuration label/source/index, the original `hipImplantConfig` name, and the screenshot folder path. The `Users` sheet differentiates between the number of unique cases and the total configuration count per planner, making it easy to spot planners who experimented with multiple implants for the same patient.
...
- Implant-specific rotations are applied automatically: detected Mathys or Medacta AMISTEM stems get `--post-rotate-z-180`, while Johnson Corail/Actis stems get both `--pre-rotate-z-180` and `--post-rotate-z-180` so the loader receives the same arguments you would pass manually.
